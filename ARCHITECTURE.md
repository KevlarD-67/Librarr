...
**Status:** Beta. The README at the repo root warns features may be broken or
incomplete (`README.md:10`).
...
The product currently versions itself at `0.3.28` (`azure-pipelines.yml:12`).
...
- `Datastore/MigrationController.cs` — runs migrations on startup using a
  ...
  - There are ~40+ migrations as of writing — a working schema-evolution
    discipline.
...
- **Class components dominate.** Most pages and complex components are class
  ...
  Newer code uses hooks — there are ~141 hook callsites
...
- `majorVersion: '0.3.28'` — the *real* shipping version.
- `minorVersion: $[counter('minorVersion', 1)]` — auto-incremented.
- `dotnetVersion: '6.0.421'` — .NET 6, not 8.
...
| `README.md`           | Marketing / install / community links; flags beta status      |
...
10. **Beta scope and missing features.** The README itself says "features may
    be broken or incomplete" — there is no in-repo issue/feature register
    beyond GitHub Issues. Triage is centralized at the Servarr level.
